...
use common::types::{
    ChatCommand, ChatEvent, ChatRequest, ChatResponse, Command, Event, NodeCommand, NodeEvent,
    ServerType,
};
        ...
        if let Ok(msg) = serde_json::from_slice::<ChatRequest>(&msg) {
            match msg {
                ChatRequest::ServerTypeQuery => {
                    ...
                    if let Ok(res) = serde_json::to_vec(&ChatResponse::ServerType {
                ...
                ChatRequest::RegistrationToChat { client_id } => {
                    ...
                    if let Ok(res) = serde_json::to_vec(&ChatResponse::RegistrationSuccess) {
                        ...
                        let _ = self
                            .controller_send
                            .send(Box::new(ChatEvent::ClientRegistered {
                                client: client_id,
                                server: self.id,
                            }));
                    }
                ...
                ChatRequest::ClientListQuery => {
                    let _ = self
                        .controller_send
                        .send(Box::new(ChatEvent::ClientListQueried {
                            notification_from: self.id,
                            from,
                        }));
                    let client_list = self.registered_clients.iter().copied().collect::<Vec<_>>();
                    if let Ok(res) = serde_json::to_vec(&ChatResponse::ClientList {
                        list_of_client_ids: client_list,
                    }) {
                        let _ =
                            self.routing_handler
                                .send_message(&res, Some(from), Some(session_id));
                ...
                ChatRequest::MessageFor { client_id, message } => {
                    ...
                    if let Ok(res) = serde_json::to_vec(&ChatResponse::MessageFrom {
                        client_id: from,
                        message,
                    }) {
                        let _ = self.routing_handler.send_message(
